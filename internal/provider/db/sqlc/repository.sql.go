...
func (q *Queries) DeleteInvitation(ctx context.Context, arg DeleteInvitationParams) error {
	_, err := q.db.Exec(ctx, deleteInvitation, arg.OrganizationID, arg.InvitationID)
	return err
}
...
SELECT id, organization_id, channel_id, external_order_id, created_at, updated_at 
FROM orders
WHERE organization_id = $1
`

func (q *Queries) ListOrders(ctx context.Context, organizationID uuid.UUID) ([]Order, error) {
...
const listOrdersWithDetails = `-- name: ListOrdersWithDetails :many
SELECT o.id, o.organization_id, o.channel_id, o.external_order_id, o.created_at, o.updated_at, c.id, c.organization_id, c.provider, c.name, c.credentials, c.status, c.created_at, c.updated_at
FROM orders AS o
JOIN channels AS c ON o.channel_id = c.id
WHERE o.organization_id = $1
`

type ListOrdersWithDetailsRow struct {
	Order   Order
	Channel Channel
}

func (q *Queries) ListOrdersWithDetails(ctx context.Context, organizationID uuid.UUID) ([]ListOrdersWithDetailsRow, error) {
	rows, err := q.db.Query(ctx, listOrdersWithDetails, organizationID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListOrdersWithDetailsRow
	for rows.Next() {
		var i ListOrdersWithDetailsRow
		if err := rows.Scan(
			&i.Order.ID,
			&i.Order.OrganizationID,
			&i.Order.ChannelID,
			&i.Order.ExternalOrderID,
			&i.Order.CreatedAt,
			&i.Order.UpdatedAt,
			&i.Channel.ID,
